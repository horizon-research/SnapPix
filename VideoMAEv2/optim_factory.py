...
def get_parameter_groups(model,
    ...
    # Initialize parameter groups and settings
    pami_fix = False
    if hasattr(model, 'coded_layer') and hasattr(model.coded_layer, 'weight'):
        pami_fix = True
        # Update parameter group definitions
        parameter_group_names["coded_layer_no_decay"] = {
            "weight_decay": 0.,
            "params": ["coded_layer.weight"],  # Only use the name for tracking
            "lr_scale": model.coded_layer.weight.lr_scale  # Consistent scale value
        }
        parameter_group_vars["coded_layer_no_decay"] = {
            "weight_decay": 0.,
            "params": [model.coded_layer.weight],  # Actual param for optimizer
            "lr_scale": model.coded_layer.weight.lr_scale
        }
    ...
    for name, param in model.named_parameters():
        ...
        if pami_fix:
            if param is model.coded_layer.weight:
                    continue

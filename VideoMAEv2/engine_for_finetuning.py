...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets, _, _) in enumerate(
        ...
        # norm in gpu
        samples = (samples - K710_SSV2_MEAN) / K710_SSV2_STD

        # binning
        if binning:
            B, C, T, H, W = samples.shape
            binned = samples.view(B, C, T, H // 4, 4, W // 4, 4).mean(dim=(-1, -3))
            upsampled = torch.zeros((B, C, T, H, W), device=binned.device, dtype=binned.dtype)
            for t in range(T):
                upsampled[:, :, t] = F.interpolate(binned[:, :, t], size=(H, W), mode='bilinear', align_corners=False)
            samples = upsampled

            # import torchvision.utils as vutils
            # vutils.save_image(samples[0,0,0,:,:], "binning.png")
...
@torch.no_grad()
def validation_one_epoch(data_loader, model, device, binning=False):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        # import torchvision.utils as vutils
        # vutils.save_image(images[:,:,0,:,:], "val_img_pt.png")

                # binning
        if binning:
            B, C, T, H, W = images.shape
            binned = images.view(B, C, T, H // 4, 4, W // 4, 4).mean(dim=(-1, -3))
            upsampled = torch.zeros((B, C, T, H, W), device=binned.device, dtype=binned.dtype)
            for t in range(T):
                upsampled[:, :, t] = F.interpolate(binned[:, :, t], size=(H, W), mode='bilinear', align_corners=False)
            images = upsampled

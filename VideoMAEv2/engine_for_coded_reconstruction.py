...
def train_one_epoch(model: torch.nn.Module,
    ...
    for step, batch in enumerate(
        ...
        if update_steps >= num_iters_per_epoch:
            continue
        # assign learning rate & weight decay for each step
        it = start_steps + update_steps  # global training iteration
        ...
        # norm in model
        # images = (images - K710_SSV2_MEAN) / K710_SSV2_STD
        ...
        decode_masked_pos[:, :] = False

        with torch.no_grad():
            norm_images = (images - K710_SSV2_MEAN) / K710_SSV2_STD
        ...
        if loss_scaler is None:
            ...
            grad_norm = loss_scaler(
                ...
                update_grad=(step+1) % acc_iters == 0)
        ...
        if lr_scheduler is not None:
            lr_scheduler.step_update(start_steps + update_steps )
...
def val_one_epoch(model: torch.nn.Module,
    ...
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('psnr', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    header = 'Epoch: [{}]'.format(epoch)
    print_freq = 20
    ...
    with torch.no_grad():
        for step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            ...
            # Forward pass
            bool_masked_pos = torch.zeros_like(images_patch[:, :, 0], dtype=torch.bool).to(device)
            decode_masked_pos = torch.zeros_like(images_patch[:, :, 0], dtype=torch.bool).to(device)
            ...
            outputs = model(images, bool_masked_pos, decode_masked_pos)
            outputs = outputs * K710_SSV2_STD + K710_SSV2_MEAN

            # added 11/17 for accurate psnr
            ...
            metric_logger.update(loss=mse_loss.item())
            metric_logger.update(psnr=psnr.item(), n=batch_size)
            metric_logger.meters['psnr'].update(psnr.item(), n=batch_size)

            if log_writer is not None:
                log_writer.update(psnr=psnr.item(), head="metrics")
                log_writer.set_step()

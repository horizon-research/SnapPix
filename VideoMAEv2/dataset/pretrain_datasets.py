...
class DataAugmentationForDecorrelation(object):
    ...
    def __call__(self, images):
        ...
        return process_data, torch.tensor(1.0), torch.tensor(1.0)
...
class HybridVideoMAE(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            if 'ssv2_processed' in video_name:
                self.new_step = 2
                self.skip_length = self.new_length * self.new_step
            ...
            if total_frame < 0:
                ...
                if not self.val:
                    ...
                    frame_id_list = self._sample_val_fid(
                        duration)
                    # import ipdb; ipdb.set_trace()
                ...
                # ]
                images = [
                    Image.fromarray(video_data[vid, :, :, 0]).convert('L')
                    for vid, _ in enumerate(frame_id_list)
                ]
                # import ipdb; ipdb.set_trace()
    ...
    def _sample_val_fid(self, num_frames):
        # Calculate the interval based on evenly spaced frames
        interval = self.new_step
        
        # Sample frames uniformly without randomness
        offsets = np.array([i * interval for i in range(self.new_length)])

        offsets[offsets >= num_frames] = num_frames - 1
